...
LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  if (ctx && oscore_conf) {
    if (coap_context_oscore_server(ctx, oscore_conf)) {
      /* oscore_conf is now owned by ctx, do not free it */
      ...
      if (session) {
        ...
        /* Calculate length of kid context */
        kid_context_len = (data[0] & 0x10) ? data[1 + partial_iv_len] + 1 : 0;
        if (1UL + partial_iv_len + kid_context_len > size)
          goto cleanup;

        kid_len = (data[0] & 0x8) ? 1 : 0;
        opt_len = 1 + partial_iv_len + kid_context_len + kid_len;

        /* Ensure we have enough data for the OSCORE option */
        if (opt_len > size)
          goto cleanup;
        ...
        if (pdu) {
          /* Add OSCORE option with random fuzzer data */
          if (coap_add_option(pdu, COAP_OPTION_OSCORE, opt_len, data)) {
            /* Add remaining data as encrypted payload */
